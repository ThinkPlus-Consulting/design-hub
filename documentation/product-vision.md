...
The approved graph model spans 75 elements across 3 tiers (58 first-class nodes, 13 registries, 4 value objects) with 71 benchmarkable objects and 106 edge types. The current implementation baseline is 36 `@Node` entities, 57 SDN `@Relationship` declarations, 1 Cypher-only polymorphic edge, and 216 passing tests. See `modeling-taxonomy.md` for classification rules and `graph-object-catalog.md` for the full inventory.
...
graph LR
    ...
    BC -->|HAS_PROCESS| BP[BusinessProcess]
...
| Implementation completeness | **AMBER** (50.7% benchmarkable coverage; 36 `@Node` entities overall) |
| Attribute depth | **AMBER** (~53%) |
| Relationship coverage | **AMBER** (57 SDN + 1 Cypher relationship types implemented) |
