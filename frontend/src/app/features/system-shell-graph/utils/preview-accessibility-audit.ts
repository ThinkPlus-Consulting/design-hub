...
export function auditPreviewAccessibility(
  ...
  resolveGraphCode: (element: HTMLElement) => string | null = nearestGraphCode,
  ...
  const pushIssue = (
    ...
    const graphCode = resolveGraphCode(element);
    const key = `${level}|${rule}|${graphCode ?? ''}|${message}`;
    if (seen.has(key)) {
      return;
    }
  ...
  return issues.sort((left, right) =>
    ...
    || (left.graphCode ?? '').localeCompare(right.graphCode ?? ''),
  );
}
...
function nearestGraphCode(element: HTMLElement): string | null {
